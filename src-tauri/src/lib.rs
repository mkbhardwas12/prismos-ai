...
/// build_app_project — Write a multi-file static web-app project (html/css/js
/// modules) generated by the local model to Downloads/prismos-apps/. The spec
/// is validated hard (relative allowlisted paths only, 20-file / 1 MB caps) —
/// see app_builder.rs. 100% local; the code only ever runs in the user's own
/// browser. Audit-logged.
#[tauri::command]
async fn build_app_project(app: tauri::AppHandle, spec_json: String) -> Result<String, String> {
    let spec: app_builder::AppSpec = serde_json::from_str(&spec_json)
        .map_err(|e| format!("Invalid app spec: {e}"))?;
    let generated = app_builder::generate_app(&spec)?;

    if let Ok(app_dir) = app.path().app_data_dir() {
        let audit = audit_log::AuditLog::new(&app_dir);
        let _ = audit.append("build_app_project", "user", &generated.dir);
    }

    serde_json::to_string(&generated).map_err(|e| e.to_string())
}

/// create_text_file — Write a plain text-format file (html/md/txt/csv/json/svg)
/// generated by the local model to the user's Downloads folder. Returns
/// GeneratedFile metadata JSON. 100% local: nothing leaves the machine.
#[tauri::command]
async fn create_text_file(
    ...
    let generated = doc_generator::generate_text_file(&title, &ext, &content)?;

    if let Ok(app_dir) = app.path().app_data_dir() {
        let audit = audit_log::AuditLog::new(&app_dir);
        let _ = audit.append("create_text_file", "user", &generated.filename);
    }
